  ...
  return (
    <nav className="w-full bg-white">
      <div className="max-w-360 relative py-5 mx-auto md:py-3 lg:py-5.25 px-5 md:px-18.75 lg:px-26 flex items-center justify-between">
        ...
        <div
          className={clsx(isMenuOpen ? "block" : "hidden", "absolute -bottom-37.5 left-0 z-10 flex flex-col w-full bg-white")}
        >
          {navLinks?.links?.map((link, index) => (
            <Link href={link.href} key={index} className="py-3 border-b border-gray-300 text-center" onClick={toogleMenu}>
              <Typography
                variant="body-lg"
                className="font-heading font-medium hover:text-primary"
              >
